...
public class MainActivity extends WearableActivity implements SensorEventListener {
    ...
    public void onClickStartAccelerometer(View view) {
        ...
        sensorManager.registerListener(this, sensorGyr, SensorManager.SENSOR_DELAY_NORMAL);
        ...
        if(event.sensor.getType() == Sensor.TYPE_ACCELEROMETER){
            ...
            nodeClient.getConnectedNodes()
                    .addOnSuccessListener(new OnSuccessListener<List<Node>>() {
                        ...
                        public void onSuccess(List<Node> nodes) {
                            for(Node node : nodes) {
                                messageClient.sendMessage(node.getId(), GYROSCOPE, FloatArray2ByteArray(event.values, System.nanoTime()))
                                        .addOnSuccessListener(new OnSuccessListener<Integer>() {
                                            @Override
                                            public void onSuccess(Integer integer) {
                                                //gelukt
